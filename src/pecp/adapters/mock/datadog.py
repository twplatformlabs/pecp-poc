"""Mock adapter for PECPDatadog resources (ADPT-02, ADPT-03 — generic team-scoped placeholder per D-01 pattern)."""
...
from pecp.models.resource_spec import DatadogSpec, ResourceSpec


class DatadogMockAdapter(AdapterBase):
    """Mock adapter for PECPDatadog resources. Generic team-scoped placeholder per D-01 pattern."""
    ...
    async def provision(self, resource: ResourceSpec) -> ProvisionResult:
        ...
        spec = resource.spec
        if not isinstance(spec, DatadogSpec):
    ...
    async def deprovision(self, resource: ResourceSpec) -> ProvisionResult:
        ...
        return ProvisionResult(
            status=ResourceStatus.ready,
            activity_log=[f"Would deprovision Datadog resource for team {team}"],
        )
