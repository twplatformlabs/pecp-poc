...
# Resolved relative to this file, not to cwd — guarantees correct config
# regardless of how pytest is invoked (CR-01 fix).
...
def test_migration_upgrade_and_downgrade(tmp_path, monkeypatch):
    ...
    # WR-02: confirm reload actually rebound DATABASE_URL before calling Alembic
    ...
    try:
        ...
        # 8. Restore prior session state — guaranteed even on assertion failure (WR-01 fix)
        importlib.reload(db_module)
